...
impl<'a, 'de> de::Deserializer<'de> for &'a mut Deserializer<'a> {
    ...
    V: de::Visitor<'de>,
  {
    match self.val {
      ...
      Val::Func(_) => {
        Err(Error::UnexpectedVal(self.path.entries(), self.val.value_type()))
      }
...
fn convert_object_to_array(
  _path: &RcValPath,
  obj: &ObjValue,
) -> Result<ArrValue> {
  Ok(
    obj.iter(true).map(convert_field_to_value).filter_map(|v| v.ok()).collect(),
  )
}
...
fn convert_field_to_value(
  ...
  match value {
    Ok(val @ Val::Obj(_)) => {
      Ok(Val::Obj(merge_key_field_into_object(&val.as_obj().unwrap(), name)?))
    }
    Ok(val) => Ok(val),
    Err(err) => Err(err),
  }
...
fn merge_key_field_into_object(
  ...
  b.member(BLOCK_LABEL_FIELD.into()).value(Val::Str(StrValue::Flat(value)))?;
  Ok(b.build())
}
    ...
    let key = loop {
      ...
      match self.val.field_visibility(key.into()) {
        ...
        Some(Visibility::Hidden) => {
          return Err(Error::FieldNotVisible(
            self.path.entries(),
            key.to_owned(),
          ))
        }
...
impl<'a, 'de: 'a> de::MapAccess<'de> for MapValueMap<'a> {
    ...
    K: de::DeserializeSeed<'de>,
  {
    ...
    match self.val.field_visibility(key.clone()) {
      ...
      None => {
        return Err(Error::FieldNotFound(self.path.entries(), key.to_string()))
      }
      _ => {}
    }
    seed.deserialize(StrDeserializer::new(key.as_str())).map(Some)
  }
